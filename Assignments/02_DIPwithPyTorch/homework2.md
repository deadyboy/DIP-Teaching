...
**编码器（Encoder）**：5 层卷积，每层使用 4×4 卷积核、stride=2 逐步下采样，通道数 3→64→128→256→512→512。第一层不使用 BatchNorm（标准做法），所有编码层使用 LeakyReLU(0.2) 激活函数以允许负值梯度流动。

**解码器（Decoder）**：5 层转置卷积，逐步上采样恢复原始分辨率。使用跳跃连接（Skip Connection）将编码器对应层的特征图在通道维度拼接到解码器输入，保留更多空间细节。前三层解码器使用 Dropout(0.5) 进行正则化，防止过拟合。最后一层使用 Tanh 激活函数，使输出值域为 [-1, 1]，与训练数据归一化范围一致。

**训练配置**：L1 Loss + Adam 优化器（lr=2e-4, betas=(0.5, 0.999)），StepLR 学习率衰减（step_size=100, gamma=0.5），batch_size=4，共训练 800 个 epoch。
...
<img src="pics/pix2pix_train_early.png" alt="Pix2Pix Train Early" width="800">
