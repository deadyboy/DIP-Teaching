...
│  逐步"压缩"图像，提取高层特征                          │
│                                                      │
│  conv1: 3→8 通道,   256×256 → 128×128               │
│  conv2: 8→16 通道,  128×128 → 64×64                 │
│  conv3: 16→32 通道, 64×64   → 32×32                 │
│  conv4: 32→64 通道, 32×32   → 16×16                 │
│  conv5: 64→128通道, 16×16   → 8×8   ← 最压缩的特征   │
└─────────────────────────────────────────────────────┘
    │
    ▼
┌─────────────────────────────────────────────────────┐
...
│                                                      │
│  deconv5: 128→64 通道,  8×8   → 16×16               │
│  deconv4: 128→32 通道,  16×16 → 32×32  (拼接 conv4)  │
│  deconv3: 64→16 通道,   32×32 → 64×64  (拼接 conv3)  │
│  deconv2: 32→8 通道,    64×64 → 128×128 (拼接 conv2)  │
│  deconv1: 16→3 通道,   128×128→ 256×256 (拼接 conv1)  │
└─────────────────────────────────────────────────────┘
    │
    ▼
